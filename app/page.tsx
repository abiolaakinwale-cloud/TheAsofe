...
function Hero({ settings }: { settings: Awaited<ReturnType<typeof getSiteSettings>> }) {
  return (
    <section className="relative" style={{ backgroundColor: "var(--color-ground)" }}>
      <div className="max-w-[100rem] mx-auto px-6 lg:px-12 grid lg:grid-cols-12 gap-10 lg:gap-16 items-center lg:min-h-[720px] py-16 lg:py-20">
        <div className="lg:col-span-6 order-2 lg:order-1">
          ...
          <Reveal delay={0.05}>
            <h1 className="display text-[clamp(2.6rem,5.6vw,5.4rem)] leading-[1.04] tracking-[-0.01em] mb-8 max-w-[18ch]" style={{ color: "var(--color-ink)" }}>
              {settings.hero.title}
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="text-base lg:text-lg leading-relaxed max-w-xl mb-10" style={{ color: "var(--color-ink-soft)" }}>
              {settings.hero.body}
            </p>
          </Reveal>
          <Reveal delay={0.15}>
            <div className="flex flex-wrap items-center gap-4">
              ...
              <Link
                href="/sellers"
                className="inline-flex items-center gap-3 px-8 py-4 text-[12px] tracking-[0.22em] uppercase font-medium border transition-colors hover:bg-[var(--color-cream)]"
        ...
        <div className="lg:col-span-6 order-1 lg:order-2 relative aspect-[4/5] lg:aspect-[3/4] lg:max-h-[680px]">
...
const stats: { stat: string; label: string }[] = [
  { stat: "8+",     label: "Independent African designers" },
  { stat: "UK",     label: "Fulfilled from our London hub" },
  { stat: "2–3 d",  label: "Average delivery to UK addresses" },
...
// ─── Why shop Asofe — shopper benefit cards ─────────────────────────────────

const shopperBenefits: { title: string; body: string; icon: ReactNode }[] = [
  { title: "Curated African designers",        body: "Independent houses from Lagos, Marrakech, Dakar, and beyond — vetted by hand.", icon: <IconAtelier /> },
  { title: "Fast UK delivery",               body: "2–3 day delivery on stocked pieces from our London fulfilment hub.",            icon: <IconTruck /> },
  { title: "UK returns address",             body: "28-day complimentary returns handled locally — no cross-border friction.",       icon: <IconReturn /> },
  { title: "Authenticity guaranteed",        body: "Every piece arrives with provenance from its atelier of origin.",                icon: <IconShield /> },
  { title: "Secure payment, GBP checkout",   body: "Pay in pounds. Cards, Apple Pay, Google Pay — handled by Stripe.",               icon: <IconCard /> },
];

function WhyShop() {
  return (
    <section className="py-20 lg:py-28 border-t" style={{ backgroundColor: "var(--color-cream)", borderColor: "var(--color-rule)" }}>
      <div className="max-w-[100rem] mx-auto px-6 lg:px-12">
        <Reveal>
          <div className="text-center mb-14 lg:mb-20">
            <p className="eyebrow mb-3" style={{ color: "var(--color-emerald)" }}>Shop with Asofe</p>
            <h2 className="display text-[clamp(1.8rem,3.4vw,3rem)] max-w-[24ch] mx-auto" style={{ color: "var(--color-ink)" }}>
              Diaspora-trusted commerce, built around the wardrobe.
            </h2>
          </div>
        </Reveal>
        <Stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-8">
          {shopperBenefits.map(b => (
            <StaggerItem key={b.title} className="text-center p-6">
              <span className="inline-flex items-center justify-center w-12 h-12 mb-6 rounded-full" style={{ backgroundColor: "var(--color-ground)", color: "var(--color-ink)" }}>
                {b.icon}
              </span>
              <h3 className="serif text-lg lg:text-xl mb-3 leading-snug" style={{ color: "var(--color-ink)" }}>
                {b.title}
              </h3>
        ...
        <div className="lg:col-span-6 relative aspect-[4/5] lg:aspect-[5/6]">
          <Reveal className="absolute inset-0">
            <div className="absolute inset-0 overflow-hidden">
              <Image
                src="/asofe/sellers-secondary.png"
